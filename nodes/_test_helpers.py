"""Shared test-only AxiomContext stub, factored out of every node's *_test.py
so each test file states only what's specific to that node (the fixture
itself, golden expectations, the independent-oracle math). Not part of the
package's runtime code path.
"""


class _TestContext:
    """Minimal AxiomContext implementation for unit tests.

    Override secrets_map with a dict to supply specific secrets your node
    requires, e.g. _TestContext(secrets_map={"OPENAI_KEY": "sk-test"}).
    """

    class _Logger:
        def debug(self, msg: str, **attrs) -> None: pass
        def info(self, msg: str, **attrs) -> None: pass
        def warn(self, msg: str, **attrs) -> None: pass
        def error(self, msg: str, **attrs) -> None: pass

    class _Secrets:
        def __init__(self, m: dict) -> None:
            self._m = m or {}
        def get(self, name: str):
            v = self._m.get(name)
            return (v, True) if v is not None else ("", False)

    def __init__(self, secrets_map: dict | None = None) -> None:
        self.log = self._Logger()
        self.secrets = self._Secrets(secrets_map or {})
        self.execution_id = "test-execution-id"
        self.flow_id = "test-flow-id"
        self.tenant_id = "test-tenant-id"
