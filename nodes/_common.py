...
# Max encoded FIT bytes accepted (inline or fetched) before any decode. FIT is a
# compact binary format; even a multi-hour, multi-sensor activity is well under
# this, so the cap exists purely to bound cost, not to accommodate real files.
MAX_FIT_BYTES = 10 * 1024 * 1024
# Timeout for a URL fetch, in seconds.
_FETCH_TIMEOUT = 20
# Max redirect hops to follow (each re-validated).
_MAX_REDIRECTS = 5

# Record extraction bounds. Measured empirically against the platform's
# deployed-invocation payload ceiling (~1 MiB hard cap, ~640 KiB treated as
# the safe target): a fully-populated Record serializes to ~380 bytes over
# the JSON bridge, so MAX_RECORD_LIMIT=1500 -> ~568 KiB (comfortable margin
# under 640 KiB) and DEFAULT_RECORD_LIMIT=1000 -> ~379 KiB. A naive default
# like "5000" or "20000" (nice round numbers with no basis in the actual
# wire size) would silently blow the transport cap on any real multi-hour
# activity -- caught by measuring an actual response, not by guessing.
DEFAULT_RECORD_LIMIT = 1000
MAX_RECORD_LIMIT = 1500
# Records embedded inside the combined ParseActivity response (bundled
# alongside file_id/sessions/laps/device_info in one payload -- kept at the
# same order of magnitude as DEFAULT_RECORD_LIMIT since it shares the same
# payload budget with the other sections).
PARSE_ACTIVITY_RECORD_CAP = 1000

# Defensive bound on sessions/laps/device_info list length. Real FIT files
# have at most a few dozen of any of these; this exists only to keep a
# pathologically constructed file (many fake session/lap/device_info
# messages) from producing an oversized response, not because any real
# activity approaches it.
MAX_SUMMARY_MESSAGES = 5000
# Defensive cap on total FIT messages iterated in a single decode pass, so a
# pathological file built from many tiny compressed-timestamp record headers
# (a small byte count producing an outsized message count) cannot make a
# single invocation iterate unboundedly. A real activity file never comes
# close to this.
MAX_MESSAGES_ITERATED = 2_000_000
...
def _fetch(url: str) -> bytes:
    ...
    redirects and reads at most `MAX_FIT_BYTES`.
    """
    current = url
    for _ in range(_MAX_REDIRECTS + 1):
        ...
        try:
            ...
            if resp.status in (301, 302, 303, 307, 308):
                ...
                resp.read(MAX_FIT_BYTES + 1)  # drain, bounded
                if not loc:
        ...
        if len(raw) > MAX_FIT_BYTES:
            raise FitDecodeError("LIMIT_EXCEEDED", "fetched file exceeds the 10 MB limit")
        return raw
...
def raw_bytes(fit_input) -> bytes:
    ...
    fetch `url`), enforcing the byte cap. Raises FitDecodeError on
    empty/oversized input. Always returns a plain `bytes` object — NEVER a
    `str` — because python-fitparse's file-open helper treats a `str`
    argument as a filesystem PATH (open(path)), not file contents; passing
    it a string would be a path-traversal / arbitrary-local-file-read
    vector. Callers must wrap this in io.BytesIO before handing it to
    fitparse, never pass it as-is."""
    data = bytes(fit_input.data)
    if data:
        if len(data) > MAX_FIT_BYTES:
            raise FitDecodeError("LIMIT_EXCEEDED", "input FIT file exceeds the 10 MB limit")
        return data
    if fit_input.url:
