...
from nodes._common import PARSE_ACTIVITY_RECORD_CAP
...
def test_parse_activity_caps_records_and_reports_true_total():
    ax = _TestContext()
    n = PARSE_ACTIVITY_RECORD_CAP + 50
    fixture = build_records_only_fixture(n)
    out = parse_activity(ax, FitInput(data=fixture))
    assert out.ok is True
    assert len(out.records) == PARSE_ACTIVITY_RECORD_CAP
